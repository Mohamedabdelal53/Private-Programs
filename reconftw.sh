		...
		if [[ $AXIOM != true ]]; then
			...
			for crit in "${severity_array[@]}"; do
				...
				axiom-scan .tmp/webs_subs.txt -m nuclei \
					...
					-H "X-Bugcrowd-Username: mohamed53@bugcrowdninja.com"
					-severity "$crit" -nh -rl "$NUCLEI_RATELIMIT" \
					-retries 2 "$NUCLEI_EXTRA_ARGS" -j -o "nuclei_output/${crit}_json.txt" "$AXIOM_EXTRA_ARGS" 2>>"$LOGFILE" >/dev/null
				# Parse the JSON output and save the results to a text file
				if [[ -s "nuclei_output/${crit}_json.txt" ]]; then
					jq -r '["[" + .["template-id"] + (if .["matcher-name"] != null then ":" + .["matcher-name"] else "" end) + "] [" + .["type"] + "] [" + .info.severity + "] " + (.["matched-at"] // .host) + (if .["extracted-results"] != null then " " + (.["extracted-results"] | @json) else "" end)] | .[]' nuclei_output/${crit}_json.txt >nuclei_output/${crit}.txt
		...
		if [[ -s "webs/webs_all.txt" ]]; then
			...
			if [[ $AXIOM != true ]]; then
				...
				if [[ $diff_webs != "0" ]] || [[ ! -s ".tmp/katana.txt" ]]; then
					if [[ $URL_CHECK_ACTIVE == true ]]; then
						if [[ $DEEP == true ]]; then
							axiom-scan webs/webs_all.txt -m katana -jc -kf all -d 3 -fs -H ${AUTH_HEADER} rdn --max-runtime 4h -o .tmp/katana.txt "$AXIOM_EXTRA_ARGS" 2>>"$LOGFILE" >/dev/null
						else
							axiom-scan webs/webs_all.txt -m katana -jc -kf all -d 2 -fs -H ${AUTH_HEADER} rdn --max-runtime 3h -o .tmp/katana.txt "$AXIOM_EXTRA_ARGS" 2>>"$LOGFILE" >/dev/null
						fi
					fi
				fi
			fi
...
function url_gf() {
	...
	if { [[ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ]] || [[ $DIFF == true ]]; } && [[ $URL_GF == true ]] &&
		...
		# Ensure .tmp/url_extract_tmp.txt exists and is not empty
